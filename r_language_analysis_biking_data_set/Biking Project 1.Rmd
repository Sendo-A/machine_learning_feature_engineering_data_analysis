...
rmseal<-rmse(ordinal_Predict_aléatoire, test_set_aléatoire$cnt)
print(rmseal)
```

```{r}
ordinal_Predict_systématique <- hot_mdl_systématique %>% predict(test_set_systématique)
rmsesys<-rmse(ordinal_Predict_systématique, test_set_systématique$cnt)
print(rmsesys)
```

```{r}
ordinal_Predict_stratifié <- ordinal_mdl_stratifié %>% predict(test_set_stratifié)
rmsestra<-rmse(ordinal_Predict_stratifié, test_set_stratifié$cnt)
print(rmsestra)
```

```{r}
hot_Predict_aléatoire <- hot_mdl_aléatoire %>% predict(test_set_aléatoire)
rmsealh<-rmse(hot_Predict_aléatoire, test_set_aléatoire$cnt)
print(rmsealh)
```
```{r}
hot_Predict_systématique <- hot_mdl_systématique %>% predict(test_set_systématique)
rmsesysh<-rmse(hot_Predict_systématique, test_set_systématique$cnt)
print(rmsesysh)
```

```{r}
hot_Predict_stratifié <- hot_mdl_stratifié %>% predict(test_set_stratifié)
rmsestrah<-rmse(hot_Predict_stratifié, test_set_stratifié$cnt)
print(rmsestrah)
...
# le model qui represente le plus les données en, ce basant sur la valeur du "rmse"

```

```{r}
#Le coefficient de corrélation entre les valeurs prévues et observées. Aide R : utilisr la fonction
cor().
```

```{r}
coral<-cor(ordinal_Predict_aléatoire,test_set_aléatoire$cnt)
print(coral)
```
```{r}
corsys <-cor(ordinal_Predict_systématique,test_set_systématique$cnt)
print(corsys)
```
```{r}
corstra<-cor(ordinal_Predict_stratifié,test_set_stratifié$cnt)
print(corstra)
```

```{r}
coralh<-cor(hot_Predict_aléatoire,test_set_aléatoire$cnt)
print(coralh)
```

```{r}
corsysh<-cor(hot_Predict_systématique,test_set_systématique$cnt)
print(corsysh)
```

```{r}
corstrah<-cor(hot_Predict_stratifié,test_set_stratifié$cnt)
print(corstrah)
```

```{r}
# 7 Conclusion générale du projet
# 7.1 Récap des résultats

best_aleatoire_simple<-c(summary(ordinal_mdl_aléatoir)$r.squared,coral,rmseal,summary(ordinal_mdl_aléatoir)$sqr)
best_aleatoire_systematique<-c(summary(ordinal_mdl_systématique)$r.squared,corsys,rmsesys,summary(ordinal_mdl_systématique)$sqr)
best_aleatoire_stratifie<-c(summary(ordinal_mdl_stratifié)$r.squared,corstra,rmsestra,summary(ordinal_mdl_stratifié)$sqr)
best_aleatoire_simple_onehot<-c(summary(hot_mdl_aléatoire)$r.squared,coralh,rmsealh,summary(hot_mdl_aléatoire)$sqr)
best_aleatoire_systematique_onehot<-c(summary(hot_mdl_systématique)$r.squared,corsysh,rmsesysh,summary(hot_mdl_systématique)$sqr)
best_aleatoire_stratifie_onehot<-c(summary(hot_mdl_stratifié)$r.squared,corstrah,rmsestrah,summary(hot_mdl_stratifié)$sqr)
best_aleatoire_simple<-as.data.frame(best_aleatoire_simple)
best_aleatoire_systematique<-as.data.frame(best_aleatoire_systematique)
best_aleatoire_stratifie<-as.data.frame(best_aleatoire_stratifie)
best_aleatoire_simple_s<-as.data.frame(best_aleatoire_simple_onehot)
best_aleatoire_systematique_s<-as.data.frame(best_aleatoire_systematique_onehot)
best_aleatoire_stratifie_s<-as.data.frame(best_aleatoire_stratifie_onehot)
Libelle <- c("R2 ajusté","RMSE","coefficient de correlation")
finalresult <- cbind(Libelle,best_aleatoire_simple,best_aleatoire_systematique,best_aleatoire_stratifie,best_aleatoire_simple_s,best_aleatoire_systematique_s, best_aleatoire_stratifie_s)
finalresult
print(finalresult)
```
...
```{r}
# 7 Conclusion générale du projet
# 7.2 (1) En ce basant sur les resultat des modeles il apparait que certain modeles ne necessitent pas toutes les variables
```


```{r}
# 7.2 (2) D'après les résultats dans les sections précédentes, l'échantionnage stratifié est beaucoup plus performent que celui systèmatique, chose qui s'applique à celui dernier à l'égard de celui aléatoire simple.
